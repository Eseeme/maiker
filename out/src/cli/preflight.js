...
import { detectAvailableProviders, explainChoice, getRoleLabel, recommendModels, getAvailableProviderNames, } from '../core/models/index.js';
...
export async function showPreflight(opts) {
    ...
    const configuredProviders = new Set(Object.values(config.models).map((m) => m.provider));
    const missingKeys = [];
    ...
    if (missingKeys.length > 0) {
        ...
        if (available.length > 0) {
            ...
            for (const [role, mc] of Object.entries(config.models)) {
                if (missingKeys.includes(mc.provider)) {
                    const alt = recommended[role];
                    const reason = explainChoice(alt, role);
                    console.log(`    ${getRoleLabel(role).padEnd(24)} → ${providerColour(alt.provider)} ${alt.model} ${chalk.gray(`(${reason})`)}`);
                }
            }
            console.log('');
    ...
    const labelWidth = Math.max(...roles.map(([k]) => getRoleLabel(k).length)) + 2;
    for (const [key, mc] of roles) {
        const label = getRoleLabel(key).padEnd(labelWidth);
        const prov = providerColour(mc.provider);
        const model = chalk.white(mc.model);
        const reason = chalk.gray(`(${explainChoice(mc, key)})`);
    ...
    // ── Prompt ────────────────────────────────────────────────────────────────
    while (true) {
        const answer = await askChoice('  Proceed with these settings? [Y/n/e] ', ['y', 'n', 'e'], 'y');
        if (answer === 'y')
...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function wrapList(items, maxWidth, indent) {
    const lines = [];
    let current = indent;
...
function askChoice(prompt, valid, defaultChoice) {
