...
function readFromMacKeychain() {
    try {
        const raw = execSync('security find-generic-password -s "Claude Code-credentials" -w', { encoding: 'utf-8', timeout: 5000, stdio: ['pipe', 'pipe', 'pipe'] }).trim();
 ...
 */
export function detectOAuthToken() {
    if (platform() === 'darwin') {
...
export function applyOAuthToken() {
    ...
    if (result.hoursLeft !== undefined && result.hoursLeft <= 0)
        return;
