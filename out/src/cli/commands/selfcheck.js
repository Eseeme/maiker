...
export function createSelfcheckCommand() {
    return new Command('selfcheck')
        ...
        results.push(runCheck('Config schema', () => {
            ...
            if (!raw.trim())
                throw new Error(`Config file is empty: ${found}`);
            try {
                yaml.load(raw);
            }
            catch (e) {
        ...
        results.push(runCheck('dist/ permissions', () => {
            ...
            try {
                ...
                if (opts.fix) {
                    throw new Error('dist/ is not writable (owned by root). Run: sudo chown -R $(whoami) ' + distPath);
                }
                throw new Error('dist/ is not writable. Fix: sudo chown -R $(whoami) ' + distPath);
            }
        }));
        // ── 7. Authentication ─────────────────────────────────────────────
