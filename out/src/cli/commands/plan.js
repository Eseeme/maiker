...
export function createPlanCommand() {
    return new Command('plan')
        ...
        try {
            ...
            if (spinner)
                spinner.text = 'Classifying task...';
            const classification = classifyTask(goal);
            let plan = buildFallbackPlan(goal, classification);
            try {
                if (spinner)
                    spinner.text = 'Running planner agent...';
                const repoSummary = await summariseRepo(absPath);
                const brief = await runResearchAgent({ runId: 'plan-preview', goal, projectPath: absPath, repoSummary }, config);
                const plannerOutput = await runPlannerAgent({ runId: 'plan-preview', goal, projectPath: absPath, brief, inspection }, config);
