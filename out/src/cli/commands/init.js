...
import { detectAvailableProviders, recommendModels, getKnownProviders, getRoleLabel, explainChoice, validateProviderKey, } from '../../core/models/index.js';
    ...
    return new Command('init')
        ...
        try {
            ...
            if (!opts.skipSetup && process.stdin.isTTY) {
                ...
                const roles = Object.keys(modelRouting);
                    ...
                    const providerColor = model.provider === 'claude' ? chalk.magenta
                        : model.provider === 'openai' ? chalk.green
                            : model.provider === 'gemini' ? chalk.blue
                                : chalk.gray;
                    console.log(`    ${getRoleLabel(role).padEnd(24)} ${providerColor(model.provider.padEnd(10))} ${model.model.padEnd(24)} ${chalk.gray(`(${reason})`)}`);
                }
                console.log('');
...
function generateConfig(models) {
    const m = models ?? {
