...
function scoreModelForRole(model, role) {
    ...
    for (const need of priorities.needs) {
        if (model.strengths.includes(need))
            score += 3;
    }
    // Multimodal bonus for roles that need it
    if (priorities.prefersMultimodal && model.multimodal)
        score += 5;
    ...
    if (role === 'researchIngestion' && model.contextWindow >= 500_000)
        score += 3;
    return score;
}
/** Recommend the best model for each role given available providers */
export function recommendModels(availableProviders) {
    const available = MODEL_REGISTRY.filter(m => availableProviders.includes(m.provider));
    const roles = Object.keys(ROLE_PRIORITIES);
    const result = {};
...
export function explainChoice(model, role) {
...
export async function validateProviderKey(provider) {
    try {
        switch (provider) {
            case 'claude': {
            ...
            case 'gemini': {
                ...
                if (!key)
                    return { valid: false, error: 'GOOGLE_API_KEY not set' };
...
export async function validateConfiguredProviders(models) {
