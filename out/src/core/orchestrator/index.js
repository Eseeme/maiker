 ...
 */
import { StateGraph, Annotation, interrupt, Command, END, START, } from '@langchain/langgraph';
import { MemorySaver } from '@langchain/langgraph';
import { classifyError } from '../../types/index.js';
import { initRunFolder, updateRunState, setStage, setStatus, setAgent, saveInspection, saveClassification, savePlan, appendValidationResult, addIssue, resolveIssue, incrementRetry, getOpenIssues, } from '../state/index.js';
import { emitRunStarted, emitRunCompleted, emitRunFailed, emitStageStarted, emitStageCompleted, emitAgentInvoked, emitAgentCompleted, emitRepairStarted, emitRepairCompleted, emitEscalationTriggered, eventBus, } from '../../artifacts/events.js';
...
function arrayReducer(existing, incoming) {
    return [...existing, ...incoming];
}
/** Reducer: merge records by shallow spread */
function recordReducer(existing, incoming) {
    return { ...existing, ...incoming };
}
/**
...
const WorkflowState = Annotation.Root({
    ...
    repairHistory: Annotation({
        value: arrayReducer,
        default: () => [],
    }),
    previousFailureCount: Annotation(),
    // ── Context & decisions ──────────────
    contextUpdates: Annotation({
        value: arrayReducer,
        default: () => [],
    }),
...
function computeExecutionWaves(subtasks) {
...
function detectFileConflicts(wave) {
    const conflicts = [];
...
function mapValidatorToCategory(name) {
    switch (name) {
...
async function nodeInspect(state) {
...
async function nodeClassify(state) {
...
async function nodePlan(state) {
    ...
    try {
        ...
        if (classified.category === 'auth') {
            return { plan, stage: 'FAILED', status: 'failed', error: `Auth error: ${classified.message}` };
        }
    }
    ...
    if (state.dryRun) {
        ...
        return { plan, stage: 'DONE', status: 'done' };
    }
...
async function nodeExecute(state) {
    ...
    for (const wave of waves) {
        ...
        for (const group of groups) {
            ...
            const results = await Promise.allSettled(group.map(async (subtask) => {
...
async function nodeValidateDeterministic(state) {
    ...
    if (!fullResult.deterministic.passed) {
        ...
        for (const failed of failedValidators) {
            const existingIssue = state.issues.find(i => i.status === 'open' && i.category === mapValidatorToCategory(failed.name) && i.stage === 'VALIDATE_DETERMINISTIC');
            if (existingIssue) {
        ...
        const passedNames = new Set(fullResult.deterministic.results.filter(r => r.status === 'passed').map(r => r.name));
...
async function nodeValidateVisual(state) {
    ...
    const plan = state.plan;
    const profile = plan.validationProfile;
    const needsVisual = profile.required.some(v => ['playwright_e2e', 'screenshot_capture', 'visual_review'].includes(v));
    if (!needsVisual) {
    ...
    try {
        ...
        const openHighSeverity = visualResult.issues.filter(i => i.severity === 'high' || i.severity === 'critical');
        ...
        if (openHighSeverity.length > 0) {
            emitStageCompleted(state.runId, 'VALIDATE_VISUAL');
            return { stage: 'REPAIR', issues: allIssues, status: 'running' };
        }
        emitStageCompleted(state.runId, 'VALIDATE_VISUAL');
        return { stage: 'POST_APPROVAL_REVIEW', issues: allIssues, status: 'running' };
    }
    catch (err) {
        ...
        emitStageCompleted(state.runId, 'VALIDATE_VISUAL');
        return {
            stage: 'POST_APPROVAL_REVIEW',
            issues: [...state.issues, warningIssue],
            status: 'running',
        };
    }
}
async function nodeRepair(state) {
    ...
    const updatedRetryCounts = { ...state.retryCounts, run: runRetry };
    for (const issue of openIssues) {
    ...
    for (const issue of openIssues) {
        if (shouldEscalate(issue.id, updatedRetryCounts, state.config.policies)) {
            if (shouldAutoReplan(updatedRetryCounts, state.config.policies)) {
                ...
                emitStageCompleted(state.runId, 'REPAIR');
                return { stage: 'PLAN', status: 'running', retryCounts: updatedRetryCounts };
            }
            emitStageCompleted(state.runId, 'REPAIR');
            return { stage: 'HUMAN_ESCALATION', status: 'running', retryCounts: updatedRetryCounts };
        }
    }
    // Progress tracking
 ...
 */
async function nodeHumanEscalation(state) {
    ...
    const openIssues = await getOpenIssues(state.runId);
    emitEscalationTriggered(state.runId, `${openIssues.length} issues could not be auto-repaired`);
    ...
    const humanDecision = (decision ?? 'proceed');
    if (humanDecision === 'abort') {
        return { stage: 'FAILED', status: 'failed', error: 'Aborted by human' };
    }
    if (humanDecision === 'replan') {
        return { stage: 'PLAN', status: 'running', humanDecision };
    }
    return { stage: 'EXECUTE', status: 'running', humanDecision };
}
async function nodePostApprovalReview(state) {
    ...
    if (state.config.policies.requireHumanApproval) {
        ...
        if (humanDecision === 'abort') {
            ...
            return { stage: 'FAILED', status: 'failed', error: 'Aborted by human at approval' };
        }
        if (humanDecision === 'replan') {
            emitStageCompleted(state.runId, 'POST_APPROVAL_REVIEW');
            return { stage: 'PLAN', status: 'running', humanDecision };
...
async function nodePromote(state) {
...
export async function runWorkflow(input) {
    ...
    await initRunFolder(runId, goal, projectPath, input.flags?.configPath ?? 'maiker.config.yaml', config.artifacts.outputDir);
 ...
 */
export async function resumeWorkflow(runId, decision, config) {
    const graph = buildWorkflowGraph();
    const app = graph.compile({ checkpointer });
    await eventBus.attachRunLog(runId, config.artifacts.outputDir);
    // Resume the graph — LangGraph replays from checkpoint and
    // passes the decision as the interrupt resume value
    const finalState = await app.invoke(new Command({ resume: decision }), {
        configurable: { thread_id: runId },
    });
    await updateRunState(runId, {
        currentStage: finalState.stage,
