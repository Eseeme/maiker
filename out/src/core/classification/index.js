...
// ─── Repo Inspector ───────────────────────────────────────────────────────────
export async function inspectRepo(projectPath) {
    ...
    const hasLinting = Boolean(allDeps.eslint ||
    ...
    const hasPlaywright = Boolean(allDeps['@playwright/test']) ||
...
async function detectFramework(root) {
    ...
    if (await fs.pathExists(join(root, 'next.config.js')))
        return 'nextjs';
    if (await fs.pathExists(join(root, 'next.config.ts')))
        return 'nextjs';
    if (await fs.pathExists(join(root, 'nuxt.config.ts')))
        return 'nuxt';
    if (await fs.pathExists(join(root, 'remix.config.js')))
        return 'remix';
    return 'unknown';
}
async function detectTestFramework(root) {
    ...
    if (await fs.pathExists(join(root, 'playwright.config.ts')))
        return 'playwright';
    if (await fs.pathExists(join(root, 'jest.config.js')))
        return 'jest';
    if (await fs.pathExists(join(root, 'vitest.config.ts')))
        return 'vitest';
    return 'none';
}
async function detectRoutes(root, framework) {
    const routes = ['/'];
    if (framework === 'nextjs') {
        // Next.js app router
...
async function detectEntryPoints(root, _framework) {
    const candidates = [
        'src/app/layout.tsx',
        'src/app/page.tsx',
        'app/layout.tsx',
    ...
    for (const c of candidates) {
        if (await fs.pathExists(join(root, c)))
            found.push(c);
    }
    return found;
}
async function detectHotspots(root, _framework) {
    const hotspots = [];
