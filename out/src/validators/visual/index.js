import { runPlaywrightE2E, captureScreenshots, isPlaywrightInstalled, } from '../../tools/playwright/index.js';
...
export async function runVisualValidation(opts) {
    ...
    if (config.validators.playwright_e2e && playwrightAvailable) {
        ...
        const result = await runPlaywrightE2E(projectPath, join(runDir, 'artifacts'), config.playwright, onOutput);
        playwrightResult = {
            name: 'playwright_e2e',
        ...
        if (result.success)
            emitValidatorPassed(runId, 'playwright_e2e');
        else
            emitValidatorFailed(runId, 'playwright_e2e', 1);
    }
    // 3. Capture screenshots
    if (config.validators.screenshot_capture && playwrightAvailable) {
        emitValidatorStarted(runId, 'screenshot_capture');
        try {
            screenshotPaths = await captureScreenshots(projectPath, screenshotsDir, config.playwright, onOutput);
            screenshotResult = {
    ...
    if (config.validators.visual_review && screenshotPaths.length > 0) {
        ...
        try {
            const reviewOutput = await runVisualReviewAgent({
                runId,
                goal: '',
                projectPath,
                screenshotPaths,
                ...
                taskConstraints,
                routeMetadata: Object.fromEntries(config.playwright.routes.map((r) => [r, r])),
            }, config);
            issues.push(...reviewOutput.issues);
            const hasHighIssues = reviewOutput.issues.some((i) => i.severity === 'high' || i.severity === 'critical');
            visualReviewResult = {
                name: 'visual_review',
