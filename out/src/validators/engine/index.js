...
export async function runFullValidation(opts) {
    ...
    const deterministicFailed = deterministicResults.filter((r) => r.status === 'failed');
    const deterministic = {
        stage: 'deterministic',
    ...
    // ── Visual validators ─────────────────────────────────────────────────────
    const needsVisual = profile.required.some((v) => ['playwright_e2e', 'screenshot_capture', 'visual_review', 'ux_rules'].includes(v));
    if (!needsVisual) {
        return {
            deterministic,
    ...
    const visualPassed = visualResults.every((r) => r.status === 'passed' || r.status === 'skipped');
    return {
        deterministic,
        visual: {
