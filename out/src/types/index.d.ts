export type WorkflowStage = 'INIT' | 'INSPECT' | 'CLASSIFY' | 'PLAN' | 'EXECUTE' | 'VALIDATE_DETERMINISTIC' | 'VALIDATE_VISUAL' | 'REPAIR' | 'HUMAN_ESCALATION' | 'POST_APPROVAL_REVIEW' | 'PROMOTE' | 'DONE' | 'FAILED' | 'BLOCKED' | 'PAUSED';
export type RunStatus = 'pending' | 'running' | 'paused' | 'done' | 'failed' | 'blocked';
export type TaskType = 'mobile-responsive-redesign' | 'framework-upgrade' | 'feature-work' | 'bugfix' | 'refactor' | 'unknown';
...
export type Framework = 'nextjs' | 'react' | 'vue' | 'nuxt' | 'angular' | 'svelte' | 'remix' | 'express' | 'fastify' | 'unknown';
...
export type ValidatorName = 'install' | 'build' | 'lint' | 'typecheck' | 'unit_tests' | 'integration_tests' | 'playwright_e2e' | 'screenshot_capture' | 'visual_review' | 'ux_rules' | 'accessibility' | 'lockfile_sanity' | 'regression_tests' | 'mobile_layout_rules';
...
export type MaikerEventType = 'run_started' | 'run_completed' | 'run_failed' | 'run_paused' | 'run_resumed' | 'stage_started' | 'stage_completed' | 'agent_invoked' | 'agent_completed' | 'tool_started' | 'tool_completed' | 'validator_started' | 'validator_passed' | 'validator_failed' | 'issue_created' | 'issue_resolved' | 'repair_started' | 'repair_completed' | 'escalation_triggered' | 'context_added' | 'context_analyzed' | 'approval_requested' | 'approval_received' | 'artifact_saved';
...
export type ErrorCategory = 'transient' | 'auth' | 'code_generation' | 'validation' | 'resource' | 'dependency' | 'unknown';
